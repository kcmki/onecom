...
export async function POST(req) {
    ...
    // updating query
    try{
        await db.collection('products').updateOne({productId:productId},{$set:{name:data.name,price:data.price,quantity:data.quantity,description:data.description,images:data.images}})
        return NextResponse.json(
